...
# Define the path to the script's directory and the embeddings file.
...
def load_embeddings():
    if os.path.exists(EMBEDDINGS_FILE):
        ...
        return np.load(EMBEDDINGS_FILE, allow_pickle=True).item() # Load the dictionary of embeddings
...
def save_embeddings(embeddings_dict):
    ...
    np.save(EMBEDDINGS_FILE, embeddings_dict) # Save the updated dictionary to the file
...
def refresh_access_periods():
    ...
    for name, data in current_embeddings.items():
        ...
        if not is_undefined_period:
            try:
                ...
                if today > end_date: # Check if today's date has passed the expiration date
    ...
    if users_to_remove:
        for user_name in users_to_remove:
            del current_embeddings[user_name] # Delete the expired user
